...
const Attendance = () => {
  ...
  const percentageData = (() => {
    ...
    return sorted.map((item) => {
      ...
      return {
        date: item.date,
        percentage: Number(
          ((cumulativePresent / cumulativeTotal) * 100).toFixed(1),
        ),
      };
    });
  })();
  ...
  const presentCount = attendance.past.filter(
    (r) => r.status === "present",
  ).length;
  const overallPct = totalClasses
  ...
  const subjectNames = [
    "All",
    ...new Set(attendance.past.map((r) => r.module_name)),
  ];
  ...
  const tabPresent = filteredHistory.filter(
    (r) => r.status === "present",
  ).length;
  ...
  const handleTakeAttendance = async (sessionId) => {
    ...
    try {
      ...
      showMessage(
        "Failed to verify IP. Are you on the correct network?",
        "error",
      );
    }
  };
    ...
    canvas.toBlob(async (blob) => {
      ...
      try {
        ...
        if (res.data.Success === true) {
          ...
          showMessage(
            res.data.message || "Could not mark attendance.",
            "error",
          );
        }
      } catch (err) {
          ...
          {attendance.todays_schedule.length === 0 ? (
            ...
            <ul className="schedule-list">
              {attendance.todays_schedule.map((s) => {
                ...
                const showButton =
                  !record && now >= startTime && now <= endTime;
                ...
                if (now < startTime) {
                  ...
                  displayStatus = (
                    <button
                      ...
                      onClick={() => handleTakeAttendance(s.id)}
                    >
                      <Camera size={14} />
                      Take Attendance
                    </button>
            ...
            <div className="history-records">
              {filteredHistory.length === 0 ? (
                ...
                filteredHistory.map((p, i) => (
                  ...
                  >
                    <div>
                      <div className="history-record-module">
                        {p.module_name}
                      </div>
