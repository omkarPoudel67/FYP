...
export default function ManageGroups() {
  ...
  const openEdit = (e, g) => {
    e.stopPropagation();
    setEditTarget(g);
    setForm({ name: g.name, module: g.module || [] });
    setFormError(null);
    setModalOpen(true);
  };
  const closeModal = () => {
  ...
  const toggleModule = (id) => {
    setForm((p) => {
      const alreadySelected = p.module.includes(id);

      // REMOVE module
      if (alreadySelected) {
        return {
          ...p,
          module: p.module.filter((x) => x !== id),
        };
      }

      // ADD module (validate max 6)
      if (p.module.length >= MAX_MODULES) {
        setFormError(`A group can have a maximum of ${MAX_MODULES} modules.`);
        return p; // block update
      }
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await fetch(
        `${API_BASE}/schedule/groups/${deleteModal.id}/`,
        {
          method: "DELETE",
          headers: authHeaders,
        },
    ...
    <div className="teacher-layout">
              ...
              <tbody>
                {filtered.map((g, idx) => (
                  <tr key={g.id} className="mg-row">
                    ...
                    {/* Modules — use g.modules (objects) for display */}
                    <td>
                        ...
                        {g.modules?.length > 0 ? (
                          <>
                            {g.modules.slice(0, 4).map((m) => (
                              <span
                                key={m.id}
                                className="mg-badge mg-badge--module"
                              >
                                {m.code}
                              </span>
                            ))}
                            {g.modules.length > 4 && (
                              <span className="mg-badge mg-badge--more">
                                +{g.modules.length - 4}
                              </span>
                            )}
                          </>
                    ...
                    {/* Students — show count / 30, click to view */}
                    <td>
                      {g.student_count > 0 ? (
                        <button
                          className="mg-students-btn"
                          onClick={() => setStudentsModal(g)}
                        >
                          <span className="mg-badge mg-badge--count">
                            {g.student_count} / {MAX_STUDENTS}
                          </span>
                          <span className="mg-students-btn__label">View</span>
                        </button>
                      ) : (
                        <span className="mg-na">0 / {MAX_STUDENTS}</span>
                      )}
                    </td>

                    <td className="mg-actions">
                      <button
                        className="mg-icon-btn mg-icon-btn--edit"
                        onClick={(e) => openEdit(e, g)}
                        title="Edit / Assign Modules"
                      >
                        <svg
                          viewBox="0 0 24 24"
                          fill="none"
                      ...
                      <button
                        className="mg-icon-btn mg-icon-btn--delete"
        ...
        <div className="mg-overlay" onClick={closeModal}>
          <div className="mg-modal" onClick={(e) => e.stopPropagation()}>
            <div className="mg-modal__header">
              <h2 className="mg-modal__title">
                {editTarget ? "Edit Group" : "New Group"}
              </h2>
            ...
            <div className="mg-form-group">
              <label className="mg-label">
                Modules{" "}
                <span className="mg-label-hint">(select all that apply)</span>
              </label>
              <div className="mg-module-grid">
                {modules.map((m) => (
                  ...
                  >
                    <input
                      type="checkbox"
                      checked={form.module.includes(m.id)}
            ...
            <div className="mg-modal__footer">
              <button className="mg-btn mg-btn--ghost" onClick={closeModal}>
                Cancel
              </button>
              <button
      ...
      {studentsModal && (
        <div className="mg-overlay" onClick={() => setStudentsModal(null)}>
          <div className="mg-modal" onClick={(e) => e.stopPropagation()}>
            <div className="mg-modal__header">
              <h2 className="mg-modal__title">
                Students in {studentsModal.name}
                <span
                  style={{
                    fontSize: "13px",
                    fontWeight: 400,
                ...
                >
                  {studentsModal.student_count} / {MAX_STUDENTS}
                </span>
              </h2>
              <button
              ...
              {studentsModal.students?.length === 0 ? (
                ...
                studentsModal.students?.map((s) => (
                  <div key={s.id} className="mg-student-row">
                    ...
                    <div>
                      <div className="mg-student-name">
                        {s.full_name || s.username}
                      </div>
                      <div className="mg-student-meta">
      ...
      {deleteModal && (
        <div className="mg-overlay" onClick={() => setDeleteModal(null)}>
          <div
            className="mg-modal mg-modal--sm"
          ...
          >
            ...
            <p className="mg-modal__body">
              Delete <strong>{deleteModal.name}</strong>? All schedules linked
              to this group will also be removed.
            </p>
            <div className="mg-modal__footer">
              <button
                className="mg-btn mg-btn--ghost"
              ...
              <button className="mg-btn mg-btn--danger" onClick={handleDelete}>
                Delete
              </button>
            </div>
          </div>
