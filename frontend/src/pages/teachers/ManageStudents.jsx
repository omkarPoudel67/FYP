...
export default function ManageStudents() {
  ...
  const fetchGroups = async () => {
    try {
      const res = await fetch(`http://localhost:8000/schedule/groups/`, {
        headers: { Authorization: `Bearer ${accessToken}` },
      });
      if (!guardRes(res)) return;
      const data = await res.json();
  ...
  // ── Filter change — auto year from semester ──────────────────────
  ...
  // ── Form change — auto year from semester ────────────────────────
  const handleChange = (e) => {
    ...
    if (name === "semester") {
      ...
      setFormErrors((prev) => ({ ...prev, semester: null, year: null }));
      return;
    }
  ...
  const handleEdit = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `${API_BASE}/students/info/${selectedStudent.id}/`,
        {
          method: "PATCH",
          headers: {
            "Content-Type": "application/json",
  ...
  return (
    <div className="teacher-layout">
        ...
        <div className="ms-filters">
          ...
          {(filters.search ||
            ...
            <button
              ...
              onClick={() =>
                setFilters({ semester: "", year: "", group: "", search: "" })
              }
            >
              Clear
            </button>
              ...
              <tbody>
                {students.map((s, idx) => (
                  <tr key={s.id}>
                    ...
                    <td>
                      <code className="ms-code">{s.user.username}</code>
                    </td>
                    <td className="ms-td--muted">{s.user.email}</td>
                    <td className="ms-td--muted">{s.user.phone_number}</td>
                    <td>
                      <span className="ms-badge ms-badge--sem">
                        Sem {s.semester}
                      </span>
                    </td>
                    <td>
                      <span className="ms-badge ms-badge--year">
                        Yr {s.year}
                      </span>
                    </td>
                    <td>
                      {s.group ? (
                        <span className="ms-badge ms-badge--group">
                          {s.group.name}
                        </span>
                      ) : (
                    ...
                    <td>
                      <span
                        className={`ms-dot ${s.has_facial_data ? "ms-dot--on" : "ms-dot--off"}`}
                      >
                    ...
                    <td>
                      <div className="ms-actions">
                        <button
                          className="ms-icon-btn ms-icon-btn--edit"
                          onClick={() => openEdit(s)}
                          title="Edit"
                        >
                          <svg
            ...
            <div className="ms-modal__body">
              <div className="ms-form-row">
                <div className="ms-field">
                  ...
                  <input
                    className={`ms-input ${formErrors.first_name ? "ms-input--error" : ""}`}
                    name="first_name"
                  ...
                  {formErrors.first_name && (
                    <span className="ms-error">{formErrors.first_name}</span>
                  )}
                </div>
                <div className="ms-field">
                  <label>Last Name</label>
                  <input
                    className={`ms-input ${formErrors.last_name ? "ms-input--error" : ""}`}
                    name="last_name"
                  ...
                  {formErrors.last_name && (
                    <span className="ms-error">{formErrors.last_name}</span>
                  )}
                </div>
              </div>
              ...
              <div className="ms-field">
                <label>Email</label>
                <input
                  className={`ms-input ${formErrors.email ? "ms-input--error" : ""}`}
                  name="email"
                  type="email"
                  value={form.email}
                  onChange={handleChange}
                ...
                />
                {formErrors.email && (
                  <span className="ms-error">{formErrors.email}</span>
                )}
              </div>
              ...
              <div className="ms-field">
                ...
                <input
                  className={`ms-input ${formErrors.phone_number ? "ms-input--error" : ""}`}
                  name="phone_number"
                ...
                {formErrors.phone_number && (
                  <span className="ms-error">{formErrors.phone_number}</span>
                )}
              </div>
              ...
              <div className="ms-form-row">
                <div className="ms-field">
                  ...
                  <select
                    className={`ms-input ${formErrors.semester ? "ms-input--error" : ""}`}
                    name="semester"
                    value={form.semester}
                    onChange={handleChange}
                  >
                  ...
                  {formErrors.semester && (
                    <span className="ms-error">{formErrors.semester}</span>
                  )}
                </div>
                <div className="ms-field">
                  <label>Year</label>

                  <div className="ms-input">Year {form.year}</div>
                </div>
              </div>
              ...
              <div className="ms-field">
                <label>Group</label>
                <select
                  className={`ms-input ${formErrors.group ? "ms-input--error" : ""}`}
                  name="group"
                  value={form.group}
                  onChange={handleChange}
                >
                ...
                {formErrors.group && (
                  <span className="ms-error">{formErrors.group}</span>
                )}
              </div>
              ...
              {modal === "create" && (
                <div className="ms-field">
                  ...
                  <input
                    className={`ms-input ${formErrors.password ? "ms-input--error" : ""}`}
                    name="password"
                    type="password"
                    value={form.password}
                    onChange={handleChange}
                  ...
                  />
                  {formErrors.password && (
                    <span className="ms-error">{formErrors.password}</span>
                  )}
                  <span className="ms-hint">
                    Student will receive this via email along with their
                    username.
                  </span>
                </div>
              )}
      ...
      {modal === "delete" && selectedStudent && (
        <div className="ms-overlay" onClick={closeModal}>
          <div
            className="ms-modal ms-modal--sm"
